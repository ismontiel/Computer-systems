// hw7test.c  A test file for the memory manager.
#include "memorymgr.h"
#include <stdio.h>
#include <stdlib.h>
...
void  coalesce() {
  ...
  while(leadingTraverser < endBlock) {     // Until we reach the last block
...
void  printallocation() {
  ...
  while(traverser < endBlock) {
    if(isAllocated(traverser) == 1) {   // All Blo
...
int *nextBlock(int *p) {
  if(isAllocated(p) == 1) {   // I know that the LSB is 1
    if(*p % 4 == 3) {   // If it is marked, then the length would have added an extra 3
      return p + (*p - 3);
    } else {
    return p + (*p - 1);
    }
